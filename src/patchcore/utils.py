...
def plot_segmentation_images(
    ...
    for image_path, mask_path, anomaly_score, segmentation in tqdm.tqdm(
        ...
        axes1.axis('off')
        f2, axes2 = plt.subplots(1, constrained_layout=True)
        axes2.axis('off')
        f3, axes3 = plt.subplots(1, constrained_layout=True)
        axes3.axis('off')
        axes1.imshow(image.transpose(1, 2, 0))
        axes2.imshow(mask.transpose(1, 2, 0))
        axes3.imshow(segmentation)
        f1.set_size_inches(3 , 3)
        f2.set_size_inches(3, 3)
        f3.set_size_inches(3, 3)
        f1.savefig(os.path.join(savename.split(".")[0] + "_pre" + ".png"), bbox_inches='tight', pad_inches=0)
        f2.savefig(os.path.join(savename.split(".")[0] + "_gt" + ".png"), bbox_inches='tight', pad_inches=0)
        f3.savefig(os.path.join(savename.split(".")[0] + "_img" + ".png"), bbox_inches='tight', pad_inches=0)
